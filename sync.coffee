...
if options.sim_path.indexOf('/') is -1 then  options.sim_path = "#{dataObj.user_name}/#{options.sim_path}"
...
upload = (path_to_file, stats) ->
    ...
    serverPath = options.sim_path + '/' + path_to_file.replace(options.watch_dir, '')

    exec "#{__dirname}/upload.sh #{path_to_file} #{serverPath} #{dataObj.user_name} #{dataObj.password}", (err, stdout, stderr) ->
        if err or stderr
            console.error color(err, "red"), stderr, stdout
        else
            console.log serverPath, color("\u2192", "cyan"), "#{serverPath}"
